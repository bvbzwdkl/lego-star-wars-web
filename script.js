// Import Vue and other necessary libraries from URLs
import { createApp } from 'https://unpkg.com/vue@3.0.11/dist/vue.esm-browser.prod.js'
import { Renderer, Scene, Camera, Points, ShaderMaterial, Texture, BufferGeometry, EffectComposer, RenderPass, UnrealBloomPass, ZoomBlurPass } from 'https://unpkg.com/troisjs@0.3.0-beta.4/build/trois.module.cdn.min.js'
import { Color, MathUtils, Vector3, Clock } from 'https://unpkg.com/three@0.127.0/build/three.module.js'



window.addEventListener('DOMContentLoaded', function() {
    const starfieldSection = document.querySelector('.starfield-section');
    ...
    function showContent() {
      ...
      setTimeout(function() {
        ...
        starfieldSection.classList.remove('hidden');
...
const { randFloat: rnd, randInt, randFloatSpread: rndFS, lerp } = MathUtils;
  ...
  createApp({
    template: `
      <Renderer ref="renderer" pointer resize="window">
        <Camera :position="{ z: 0 }" :fov="50" :far="500"/>
        <Scene>
      ...
      <a href="#" @click="updateColors" @mouseenter="targetTimeCoef = 100; startSpinning();" @mouseleave="targetTimeCoef = 1; stopSpinning();">
        <img ref="logo" src="Ship.png" alt="Ship" class="logo" :class="{ 'spinning': isSpinning }">
      </a>
    ...
    data() {
      return {
        zoomStrength: 0,
        isSpinning: false,
        bannerText: 'ABCDEFGHI1323JKLMNOPQRS2321TUVWXY',
      }
    },
    mounted() {
        this.adjustSectionHeightOnInit();
        window.addEventListener('resize', this.adjustSectionHeight);
    ...
    methods: {
      ...
      spinLogo() {
        ...
        }, 2000); // Adjust the duration of the spin animation (in milliseconds)
      },
      startSpinning() {
        this.isSpinning = true;
      },
      ...
      adjustSectionHeightOnInit() {
        this.initialHeight = window.innerHeight;
        this.adjustSectionHeight();
      },
  
      adjustSectionHeight() {
        const section = document.querySelector('.starfield-section');
        if (section) {
          section.style.height = `${this.initialHeight}px`;
        }
      },
  
      // Ensure to remove the event listener when the component is unmounted
      beforeUnmount() {
        window.removeEventListener('resize', this.adjustSectionHeight);
      },
      /* animateBanner() {
        const bannerWidth = this.$el.offsetWidth;
        const textWidth = this.$el.querySelector('.banner-text').offsetWidth;
        const duration = 10000; // Adjust the duration as needed
        const startPosition = bannerWidth;
        const endPosition = -textWidth;
  
        const animation = this.$el.querySelector('.banner-text').animate(
          [
